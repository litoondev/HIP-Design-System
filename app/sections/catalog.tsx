...
export interface SectionEntry {
  /** URL segment — /sections/{slug}. Also used as the render id. */
  slug: string;
  num: string;
  name: string;
  tags: string[];
  note: ReactNode;
  Component: () => ReactNode;
  /** Set on variant designs that aren't part of the 12-item sidebar list. */
  variantOf?: string;
}

/**
 * One entry per section design — the single source both the index gallery
 * (app/sections/page.tsx) and each detail route (app/sections/[slug]/page.tsx) render from.
 * Ordered and numbered to match the homepage structure reference, same as before the split
 * into per-section pages.
 */
export const sectionEntries: SectionEntry[] = [
  {
    ...
    note: (
      <>
        ...
        Figtree 600, clamp(44px–80px), line-height 1.1. Social dots: primary-500. Gradient
        overlay: <code>var(--color-navy)</code> at 70%. Hero image: pull from Google Drive →{" "}
  ...
  {
    slug: "highlights-cream",
    num: "02B",
    name: "Highlights — Trust Badges Grid (Cream)",
    tags: ["Cream bg · 4-col × 2 rows · Icon + text"],
    variantOf: "highlights",
    note: (
      <>
  ...
  },
  {
    ...
    note: (
      <>
        <strong>DS tokens:</strong> Section bg: primary-50. Play ring: SVG textPath, cta-500 at
        70%. Play button: cta-500 with 35% orange glow. Button border: primary-500. H2: Figtree
        800, clamp(26–40px). Video image: Google Drive →{" "}
  ...
  },
  {
    ...
    note: (
      <>
        <NewDesign />
        <strong>DS tokens:</strong> Section bg: primary-50. Age pill: white bg / primary-200
        border / primary-500 text. H3: Figtree 800 24px. Row divider: primary-100. Photography
        must clearly match the segment — a teen photo under &ldquo;Kids&rdquo; reads as a
        mistake. Each segment links to its own audience landing page. Images: Google Drive →{" "}
  ...
  },
  {
    ...
    note: (
      <>
        ...
        competes with Request Free Consult. Avatar ring uses Instagram&apos;s own brand
        gradient, the single non-DS colour here. Tiles: 1:1, 4–5 visible on desktop,
        scroll-snap. Post themes: smile reveals, team culture, care tips, events, before/after.
        Images: Google Drive → <strong>Instagram Section</strong> folder.
      </>
    ),
  ...
  },
  {
    ...
    note: (
      <>
        ...
        Figtree 800 / shadow cta-500 at 35%. One CTA only — no competing links. Image: Google
        Drive → <strong>CTA Section</strong> folder.
      </>
    ),
  ...
  },
  {
    ...
    note: (
      <>
        ...
        gray-50 bg / gray-200 border. Map: primary-50 → primary-100, primary-200 border,
        cta-500 pins. Real build needs pin pop-up cards (photo, address, &ldquo;Visit This
...
export function getSectionEntry(slug: string): SectionEntry | undefined {
  return sectionEntries.find((entry) => entry.slug === slug);
}

/** The 12 primary entries the sidebar links to — variants (01B, 02B) are reachable only
 *  from within their primary entry's page, not from the sidebar submenu. */
export const primarySectionEntries = sectionEntries.filter((entry) => !entry.variantOf);
